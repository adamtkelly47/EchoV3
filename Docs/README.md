...
**Phase 12 — Schwab read integration.** Phases 0-11 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write) are complete. Phase 12 adds `domains/portfolio/` and `providers/schwab/adapter.py` — a verified portfolio source of truth (accounts, positions, balances, immutable reconciled snapshots) from a real connected Schwab account. Schwab has no separate read-only OAuth product (the grant is trade-capable regardless); read-only is enforced entirely by omission — no method anywhere in this codebase places, modifies, or cancels an order. Live testing against the real connected account (2 accounts, 14 positions, ~$16.2k) found and fixed two real bugs before this phase was considered done: ETF positions were misclassified (Schwab's broad `assetType` field doesn't distinguish them from other collective investments — the specific classification is in a separate `instrument.type` field), and a more serious data-integrity bug where every sync silently orphaned a duplicate set of position/balance rows instead of updating the same ones (caught only because live testing surfaced stale data that unit tests, using a fake with the same latent bug, could not). See `Docs/DECISION_LOG.md`'s Phase 12 entry for the full diagnosis of both, plus the real Schwab-specific OAuth quirks (7-day hard refresh-token expiry, an unreachable-by-design redirect URI requiring a copy-pasted callback URL instead of a tunnel).
