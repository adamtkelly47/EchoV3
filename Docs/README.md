...
**Phase 24 — Proactive monitoring foundation.** Phases 0-19 and 22-23 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system, financial data provider evaluation harness, research ingestion foundation, news intelligence, SEC Form 4 insider intelligence, Congressional disclosure pipeline, unified dashboard, project and task intelligence) are complete. Phases 20-21 (Gmail integration) remain deferred at the user's explicit request. Phase 24 populates the System domain for the first time (Docs/DOMAIN_OWNERSHIP.md: System owns "platform monitoring", "system alert generation", "raise alert"/"acknowledge alert") with five real background monitors — calendar conflict, stale Schwab sync, IPS concentration breach, material portfolio news, integration failure — each raised through a new `MonitoringOrchestrator` (`application/orchestrators/monitoring.py`) coordinating System + Portfolio + Calendar + Research. Verification 1 ("monitors may notify but cannot execute consequential actions") holds structurally: the orchestrator has no dependency on `ApprovalService` or any write path at all. Deduplication (verification 2) and a real `reason` field on every alert (verification 3) are enforced inside `SystemService`, not left to individual evaluators. The Phase 1 throwaway `echo:jobs:test` job is replaced with a real `JobEnvelope[MonitoringEvaluateInput]` consumed every 300 seconds from a dedicated Redis queue — live-verified end to end through the real scheduler/worker containers, not just the on-demand API path. See `Docs/DECISION_LOG.md`'s Phase 24 entry for the full design, the real architecture-rule discovery that shaped the calendar-conflict monitor, and live verification results.
