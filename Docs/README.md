...
**Phase 13 — Portfolio calculations and dashboard.** Phases 0-12 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration) are complete. Phase 13 turns Phase 12's verified, reconciled Schwab data into deterministic analysis — position weights, asset-class exposure, cross-account exposure, concentration warnings, and unrealized gain/loss — computed entirely in `domains/portfolio/policies.py` (pure functions, unit-tested) and exposed as `GET /portfolio/dashboard`. Every number traces back to the latest already-synced snapshot (no live Schwab call from the dashboard itself), and every calculation is persisted as a `core.provenance.ComputedValueRecord` so it's independently reproducible. Sector mapping is real but intentionally empty of content — Schwab's API has no sector field, real sector classification belongs to the not-yet-built Research domain (Phase 16+), and rather than fabricate a mapping the dashboard reports a single honest "Unknown" bucket with an explicit warning. Verified live against the real connected Schwab account: 14 positions, correct asset-class mix, two real symbols (`IBIT`, `SMCI`) correctly identified as held across both accounts, a genuine 42.9% concentration warning on `UNH`, and $728.26 in real unrealized gains. See `Docs/DECISION_LOG.md`'s Phase 13 entry for the full reasoning behind the sector and dashboard-scope decisions.
