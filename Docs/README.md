...
**Phase 19 — Congressional disclosure pipeline.** Phases 0-18 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system, financial data provider evaluation harness, research ingestion foundation, news intelligence, SEC Form 4 insider intelligence) are complete. Phase 19 extends `domains/research/` with Senate Periodic Transaction Report (PTR) ingestion and analysis — real, live-verified data sources found and validated before any design was committed to: the free third-party aggregator APIs this project initially expected to exist were confirmed dead (`403 AccessDenied`), the House Clerk's own disclosure system was confirmed to publish PTRs as scanned PDFs only (a documented scope limitation — Senate only, this phase), and the Senate's own eFD system was confirmed to render real, structured transaction tables for electronically-filed reports, walked end to end against a real senator's real 703-transaction filing before any parser was written. Politician identity and committee assignments are resolved against `unitedstates/congress-legislators`, a real public reference dataset — its committee-membership data is a current snapshot, not a historical time series, a real constraint this phase's design documents and respects rather than papers over (PROMPT.md verification 2). Every disclosed amount is stored as its real bracket (a low/high range, or a genuinely open-ended "Over $X"), never collapsed into a fabricated point figure, including in aggregate profile totals and anomaly comparisons (verification 1) — live-verified against a real $5M+ Williams Companies sale by a sitting senator, flagged as 1003.52x his own historical average without ever stating a specific dollar value. Committee-relationship features are a deterministic keyword overlap between a traded issuer's real industry classification (reused from Phase 16) and each committee's own real jurisdiction text, with every match fully inspectable and explicitly non-accusatory (verifications 3-4). No Model Gateway or orchestrator is used anywhere in this phase — every implement item is a deterministic computation. See `Docs/DECISION_LOG.md`'s Phase 19 entry for the full data-source discovery process and live verification results.
