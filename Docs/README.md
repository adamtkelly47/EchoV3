...
**Phase 18 — SEC Form 4 / insider transaction intelligence.** Phases 0-17 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system, financial data provider evaluation harness, research ingestion foundation, news intelligence) are complete. Phase 18 extends `domains/research/` with insider transaction (Form 4) ingestion, normalization, and analysis: real SEC EDGAR XML parsing (a two-step retrieval — the listed `primaryDocument` is an XSLT-rendered HTML view, not the machine-readable XML, which required a live diagnostic pass against real UNH filings before the parser was written), deterministic transaction-type/planned-sale classification, code-computed transaction values and ownership-change percentages, personal-baseline anomaly features (size-vs-own-history, same-direction insider clustering) each stating their own comparison baseline, Ollama footnote classification, and an explicitly opt-in Claude interpretation step whose prompt directly enforces CONSTITUTION.md's Verified Truth section (forbidding accusatory language, separating verified facts from computed features) — coordinated by a new `application/orchestrators/insider_intelligence.py`. Live verification against real UNH filings caught a real idempotency bug before it shipped: a random `transaction_id` meant any re-sync (which always re-fetches the most recent N filings) would duplicate already-ingested rows and corrupt every profile/anomaly baseline; fixed by deriving the id deterministically from `(accession_number, index-within-filing)`, then re-verified live that two full ingestion passes produce identical stored counts. See `Docs/DECISION_LOG.md`'s Phase 18 entry for the full pipeline design, the live schema-discovery findings, and the idempotency bug.
