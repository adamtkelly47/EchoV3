...
**Phase 15 — Financial data provider evaluation harness.** Phases 0-14 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system) are complete. Phase 15 built `scripts/provider_evaluation/` — a live-testing harness that evaluates candidate external research-data providers (not Schwab; Phase 12 already covers portfolio data) against PROMPT.md's 13 measured criteria and 8 research needs. Deliberately **not** a domain or `providers/` adapter — PROMPT.md Phase 15 is explicit that no permanent provider is selected yet. Every pass/fail outcome came from an actual live request, never a provider's own marketing claim; criteria that can't be answered from a single script run (documentation quality, reliability, licensing, schema stability, cost) are marked as such rather than scored. Live-tested against real Finnhub and FMP API keys plus the keyless SEC EDGAR and Senate/House Stock Watcher public sources, producing genuinely useful findings: Finnhub's free tier works well except its candles endpoint (403, paywalled); FMP's entire v3 API surface is deprecated for this key ("Legacy Endpoint" 403s); SEC EDGAR works cleanly; the once-public congressional-disclosure S3 buckets now return 403 AccessDenied; Reddit blocks unauthenticated access. See the dated report at `Docs/decisions/PROVIDER_EVALUATION_2026-07-21.md` and `Docs/DECISION_LOG.md`'s Phase 15 entry, which also documents a real false-positive bug found and fixed in the harness's own `symbol_coverage` check before its output was trusted.
