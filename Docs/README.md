...
**Phase 10 — Google Calendar read integration.** Phases 0-9 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation) are complete. Phase 10 adds `domains/calendar/`, `providers/calendar/google/` (plain `httpx` against Google's real, documentation-verified REST APIs — OAuth, calendar listing, event listing/search/detail, free/busy), and `infrastructure/secrets/encryption.py` (Fernet-based encryption at rest for OAuth tokens, required by `Docs/SECURITY.md` and built fresh this phase). Read-only scope only — no write path exists yet. Verified live against a real connected Google account: real events, correct recurring-instance linkage, correct timezone handling, live token-refresh recovery, and honest error responses (a global `EchoError` -> HTTP exception handler, added this phase, fixed a bare-500 gap that had existed silently since Phase 6). See `Docs/DECISION_LOG.md`'s Phase 10 entry for the OAuth debugging chain (network tunneling, OAuth client type) and the architecture decisions.
