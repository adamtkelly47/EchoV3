...
**Phase 25 — Evaluation and trust dashboard.** Phases 0-19 and 22-24 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system, financial data provider evaluation harness, research ingestion foundation, news intelligence, SEC Form 4 insider intelligence, Congressional disclosure pipeline, unified dashboard, project and task intelligence, proactive monitoring foundation) are complete. Phases 20-21 (Gmail integration) remain deferred at the user's explicit request. Phase 25 measures whether Echo is actually becoming reliable, computing all 13 of PROMPT.md's tracked metrics (tool accuracy, calculation reconciliation, data freshness, local model schema success, local model classification quality, Claude escalation rate, hallucination incidents, approval bypass attempts blocked, execution verification rate, integration uptime, user corrections, cost, latency) almost entirely from signals that already existed in this codebase but had never been read — `model_calls`/`tool_calls` (`infrastructure/database/repositories/observability.py`) have existed since Phase 7 but nothing had ever written to `model_calls` or read from either table until now. `providers/models/gateway.py`'s `ModelGateway` now records every real model call using data its adapters already compute (no new pricing/timing logic needed); a new `application/queries/trust_dashboard_query.py`'s `TrustDashboardQueryService` (the second Application-layer query, after Phase 22's dashboard) aggregates everything into `GET /trust/dashboard`. A real, one-line audit gap was found and fixed along the way: `domains/approvals/service.py`'s `execute()` recorded an audit event on every rejection branch except the most important one (attempting to run something never approved) — now it does. "Hallucination incidents" and "create regression datasets from corrected failures" are new, deliberately human-triggered System-domain concepts (`POST /trust/hallucination-incidents`) — no automatic hallucination detector exists or is claimed anywhere in this codebase. See `Docs/DECISION_LOG.md`'s Phase 25 entry for the full design and live verification results.
