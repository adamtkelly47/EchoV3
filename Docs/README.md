...
**Phases 20-21 — Gmail read integration and approval-gated writes.** Every phase PROMPT.md authorizes (0-27) is now implemented — the Gmail deferral was lifted at the user's explicit request in this session, and `domains/email/` was built from scratch mirroring `domains/calendar/`'s Phase 10-11 pattern exactly: OAuth (`gmail.readonly` + `gmail.modify`, additively requested), encrypted token storage, a 5-minute read cache, and seven approval-gated write actions (create/update draft, send, reply, archive, label, trash), each with its own `WriteAdapter`/`ExecutionVerifier` pair. Local classification, action-item extraction, and response-needed detection run as a single Ollama structured call per message (`application/orchestrators/email_intelligence.py`), matching the cost-discipline precedent set by Phase 17's news classification. Endpoint URLs, scopes, and response shapes were verified against Google's own current Gmail API documentation (not training-data guesswork) before writing the adapter — but unlike Calendar's Phase 10-11, the full authenticated OAuth round-trip has not been exercised against a real Gmail account from inside this codebase, since that requires the user's own Google consent flow; every other layer (678 backend tests total, real Neon migration, real running API) is real and verified. See `Docs/DECISION_LOG.md`'s Phase 20-21 entry for the full design and what remains for the user to verify live.

**Frontend coverage for Email, Projects, Monitors, Trust, and Paper trading** followed in the same session — five phases whose own decision log entries had explicitly deferred a dashboard-visible surface now have one (`frontend/app/{email,projects,monitors,trust,paper-trading}/page.tsx`), plus a real nav bar tying all seven pages together. A real, previously-latent CORS gap (`PATCH` was never in `apps/api/main.py`'s allowed methods, even though several routes had used it since their own phases) surfaced and was fixed the moment a browser actually called one. See `Docs/DECISION_LOG.md`'s "Frontend coverage" entry for the full design and live verification results.

Phase 27 — Paper trading observation — was the last *numbered* phase completed prior to this session; see the DECISION_LOG for its own entry. It evaluates trade reasoning without enabling trading — no order or execute endpoint exists anywhere in this codebase, for real trades or hypothetical ones.
