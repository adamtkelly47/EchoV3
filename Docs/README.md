...
**Phase 17 — News intelligence.** Phases 0-16 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read/write, Schwab read integration, portfolio calculations and dashboard, IPS system, financial data provider evaluation harness, research ingestion foundation) are complete. Phase 17 extends `domains/research/` with a full news pipeline — ingestion, deterministic deduplication and relevance scoring (source quality × event-type materiality × recency decay × a real portfolio-holding boost), Ollama event-type classification (a closed enum the model structurally cannot escape) and per-article summarization, and a final Claude-synthesized narrative that cites back to its sources — coordinated by a new `application/orchestrators/news_intelligence.py`, since it's the first pipeline in this codebase needing both the Model Gateway and a second domain (Portfolio) at once. This phase's first live run was also this codebase's first-ever live call to Claude, and it immediately found two real bugs neither the (fully passing) unit test suite nor any prior phase could have caught: Claude rejected every request outright (`temperature` is deprecated for the configured model — fixed in `providers/models/claude/adapter.py`, a latent bug since Phase 7), and a prompt/parser mismatch was silently and successfully falling back to "summary = headline" for every article. Verified live against the real portfolio's largest holding (UNH, 42.92%): 489 real articles ingested, 30 genuine near-duplicates correctly collapsed (including four analysts' price-target notes on the same day), 69 correctly suppressed as low-relevance, and a real Claude narrative citing `[1]`-`[5]` back to the specific numbered articles it was given. See `Docs/DECISION_LOG.md`'s Phase 17 entry for the full pipeline design and both bugs' root causes.
