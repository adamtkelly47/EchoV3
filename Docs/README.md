...
**Phase 11 — Calendar approval-gated writes.** Phases 0-10 (governing documents, Docker foundation, quality/CI, core runtime contracts, database foundation, capability registry, approval engine, model gateway, conversation loop, memory foundation, Google Calendar read integration) are complete. Phase 11 adds narrow, safe calendar modifications — create/modify/delete event — routed entirely through the Phase 6 Approval Engine (`application/orchestrators/calendar_writes.py`, `domains/calendar/write_adapters.py`). No shortcut exists: every write is proposed, reviewed (full payload visible), explicitly approved, then executed with post-execution verification (a fresh, independent re-read from Google, not just trusting the write response). Recurring-event edits require an explicit scope (single instance vs. entire series) — "this and following" is deliberately not supported, since Google's API only offers it as a two-call, partially-completable workaround. Verified live against a real connected Google account through a full create -> approve -> execute -> duplicate-execute -> modify -> delete -> verify-cancelled cycle. See `Docs/DECISION_LOG.md`'s Phase 11 entry for the OAuth re-scoping (existing connections needed to re-authorize under a broader scope) and Google's real (and non-obvious) cancelled-not-404 delete behavior.
